...
import com.zl.frame.add.NatAddFrame;
...
public class NatListPanel extends JPanel {
    ...
    public NatListPanel() {
        ...
        updateBtn.addActionListener(e -> {
            ...
            if (selectedRow == -1) {
                ...
                new NatAddFrame(String.valueOf(id));
            }
        });
        ...
        deleteBtn.addActionListener(e -> {
            ...
            if (selectedRow == -1) {
                ...
                Object id = jTable.getModel().getValueAt(selectedRow, 0);
                int i = JOptionPane.showConfirmDialog(null, "Delete or not?");
                if (i == 0) {
                    NatDao.delete(String.valueOf(id));
                    reload();
                }
            }
        });
    ...
    private void reload() {
        String[] columnName = {"number", "name","collage", "test time", "result"};

        List<Nat> list = NatDao.list();
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            rowData[i][2] = list.get(i).getNdepnam();
            rowData[i][3] = list.get(i).getTime();
            rowData[i][4] = list.get(i).getRes();
        }
