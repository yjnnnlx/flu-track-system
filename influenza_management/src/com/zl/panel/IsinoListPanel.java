...
public class IsinoListPanel extends JPanel {

    private JTable jTable;

    public IsinoListPanel() {
        ...
        updateBtn.addActionListener(e -> {
            ...
            if (selectedRow == -1) {
                ...
                new IsinoAddFrame(String.valueOf(id));
            }
        });
        ...
        deleteBtn.addActionListener(e -> {
            ...
            if (selectedRow == -1) {
                ...
                if (i == 0) {
                    IsinoDao.delete(String.valueOf(id));
                    reload();
                }
            }
        });
    ...
    private void reload() {
        String[] columnName = {"number", "name","collage", "status", "mark"};
        List<Isino> list = IsinoDao.list();
        Object[][] rowData = new Object[list.size()][5];
        for (int i = 0; i < list.size(); i++) {
            rowData[i][0] = list.get(i).getId();
            rowData[i][1] = list.get(i).getName();
            rowData[i][2] = list.get(i).getIdepnam();
            rowData[i][3] = list.get(i).getIsback();
            rowData[i][4] = list.get(i).getMark();
        }
